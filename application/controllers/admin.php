    ...
    public function index() {
        ...
        if (empty($_SESSION['sessemp'])) {
            ...
            if (($_SESSION['sessemp']['status'] == 0)) {
                ...
                $data['picking'] = $this->materials_model->select_rp_id($_SESSION['sessemp']['id']);
                $data['picking_confirm'] = $this->materials_model->select_rp_confirm_id($_SESSION['sessemp']['id'], $d, $m, $Y);
...
//print_r($type);
    ...
    public function save_img() {
        ...
        if (!$this->upload->do_upload('file')) {
            ...
            if ($width > $hieght) {
                $cmd = "magick " . $input . " -quality 85  -scale 200x130 -gravity center -extent x130 " . $output . "";
                ...
                $cmd = "magick " . $input . " -quality 85  -scale 200x130 -gravity center -extent 200x " . $output . "";
                $cmd2 = "magick " . $input . " -scale 300x260^ -gravity center -extent 300x260 -quality 80 " . $output2 . "";
                $cmd3 = "magick " . $input . " -scale 300x240^ -gravity center -extent 300x240 -quality 80 " . $output3 . "";
                exec($cmd3 . " && " . $cmd2 . " && " . $cmd);
            }
    ...
    public function report_store() {
        ...
        if ($select_day == "day") {
            ...
            $day['data2'] = $this->materials_model->select_error_material();
        } else {
    ...
    public function report_store1() {
        ...
        if ($select_day == "day") {
            $day['data2'] = $this->materials_model->select_day_material1($frist_day, $end_day);
            // print_r($day
        } else if ($select_day == "month") {
            $year = date('Y', strtotime($frist_day));
            $timeDate = strtotime($end_day);
            $lastDay = date("t", $timeDate);
            $day['data2'] = $this->materials_model->select_month_material1($year, $frist_day, $end_day, $lastDay);
        } else if ($select_day == "year") {
            $year = $frist_day;
            $day['data2'] = $this->materials_model->select_year_material1($year);
        } else {
            $day['data2'] = $this->materials_model->select_error_material1();
        }
...
//        error_reporting(E_ALL);
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('E3', 'ประเภท')
                ->setCellValue('F3', 'หมวด')
                ->setCellValue('G3', 'จำนวน');
        ...
        foreach ($day['data2'] as $key) {
            $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A' . $i . '', $num)
                    ->setCellValue('B' . $i . '', $key->rp_date)
                    ->setCellValue('C' . $i . '', $key->emp_fristname)
                    ->setCellValue('D' . $i . '', $key->rp_productname)
                    ->setCellValue('E' . $i . '', $key->ty_name)
                    ->setCellValue('F' . $i . '', $key->ct_name)
                    ->setCellValue('G' . $i . '', $key->rp_amount);
        ...
        $a = $i - 1;
        $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('F' . $i . '', 'ผลรวม')
                ->setCellValue('G' . $i . '', '=SUM(G4:G' . $a . ')');
        $objPHPExcel->getActiveSheet()
                ->getStyle('F' . $i . '')
                ->getAlignment()
                ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('G' . $i . '')->applyFromArray($styleColor);
        unset($styleColor);
        $objPHPExcel->getActiveSheet()->getColumnDimension('G')->setAutoSize(true);
    ...
    public function product_search() {
        ...
        if (empty($key)) {
            ...
            $data['data'] = $this->materials_model->product_search($key);
            if (count($data['data']) == 0) {
                echo "row0";
            } else {
                $this->load->view('ajx_autocomplete', $data);
            }
        }
    }
    ...
    public function one_product() {
        ...
        $view = $this->load->view("ajx_one_product", $data, true);
        echo $view;
    }
    ...
    public function cancel_product() {
        ...
        $data['nextMonth'] = date('Y-n', mktime(0, 0, 0, ($sp_m_y[1] + 1), 1, $sp_m_y[0]));
        $data['prevMonth'] = date('Y-n', mktime(0, 0, 0, ($sp_m_y[1] - 1), 1, $sp_m_y[0]));
        $this->load->view('calendar-en', $data);
    }
    ...
    public function add_product_to_sale() {
        ...
        if ($id) {
            ...
            $data['proname'] = $this->materials_model->select_sm_id_name($sm_sale);
            $this->load->view("add_product_to_sale", $data);
        }
    }

    public function select_to_sale() {
        $this->check();
        ...
        if ($id == 0) {
            ...
            $data['row'] = $this->materials_model->select_to_sale_show($id, $sm_sale);
            $show = $this->load->view("select_to_sale_show", $data, true);
            echo $show;
        }
    }
    ...
    public function report_store_sale($offset = 0) {
        $this->check();
        if (empty($offset))
            $offset = 0;
        $config['base_url'] = base_url() . "admin/report_store_sale";
        ...
        if (!$foo['data'] = $this->cache->get($cacheID . $offset)) {
            $data['ss'] = $this->materials_model->select_ss_sm($this->limit, $offset)->result();
            $this->cache->save($cacheID . $offset, $data['ss'], 6000);
        } else {
            $data['ss'] = $this->cache->get($cacheID . $offset, 6000);
        }
    ...
    public function all_report() {
        $this->check();
        $count['mtr'] = $this->materials_model->count_materials();
        $count['mtr_app'] = $this->materials_model->count_materials_app();
        $count['rpo'] = $this->materials_model->count_report_order();
        $count['ss'] = $this->materials_model->count_store_sale();
        $count['rp'] = $this->materials_model->count_report_picking();
        $count['rp_app'] = $this->materials_model->count_report_picking_app();
        $view = $this->load->view("all_report", $count, true);
        echo $view;
    }
    ...
    public function update_picking() {
        $rp_id = $this->input->post("rp_id");
        $num = $this->input->post("num");
        $data["rp_amount"] = $num;
        $id_emp = $this->input->post("id_emp");
        echo $this->materials_model->update_re_picking_model($rp_id, $id_emp, $data);
    }

    public function delete_picking() {
        $id_emp = $this->input->get("id_emp");
        $rp_id = $this->input->get("id_autoid");
        $this->materials_model->delete_re_picking_model($id_emp, $rp_id);
        redirect('admin/approval');
    }

    function page_store($offset = 0) {
        $this->check();
        if (empty($offset))
            $offset = 0;
        $config['base_url'] = base_url() . "admin/page_store";
        ...
        $cacheID = "page_store";
        if (!$foo['data'] = $this->cache->get($cacheID . $offset)) {
            $data['sm'] = $this->materials_model->select_sm_limit($this->limit, $offset);
            $this->cache->save($cacheID . $offset, $data['sm'], 6000);
        } else {
            $data['sm'] = $this->cache->get($cacheID . $offset, 6000);
        }
        $this->load->view('all_store', $data);
    }

    function page_emp($offset = 0, $order_column = 'emp_autoid', $order_type = 'asc') {
        $this->check();

        if (empty($offset))
            $offset = 0;
        if (empty($order_column))
            $order_column = 'emp_autoid';
        if (empty($order_type))
            $order_type = 'asc';
        //TODO: check for valid column
        // load data
        $User = $this->employee_model->get_paged_list($this->limit, $offset, $order_column, $order_type)->result();

        // generate pagination
        $config['base_url'] = site_url('admin/page_emp/');
        ...
        $this->table->set_heading(
                'No', anchor(base_url('admin/page_emp/' . $offset . '/emp_autoid/' . $new_order), 'รหัส'), anchor(base_url('admin/page_emp/' . $offset . '/emp_fristname/' . $new_order), 'ชื่อ'), 'นามสกุล', 'เลขบัตรประชาชน', 'วันกิด', 'เบอร์โทรศัทพ์', 'อีเมลล์', 'ตำแหน่ง', 'ตัวเลือก'
        );
        $i = 0 + $offset;
        foreach ($User as $value) {
            $this->table->add_row( ++$i, $value->emp_autoid, $value->emp_fristname, $value->emp_lastname, $value->emp_id, $value->emp_birthday, $value->emp_phone, $value->emp_email, $value->emp_status == 0 ? 'admin' : 'employee', anchor(base_url('admin/view/?id=' . $value->emp_autoid), '<i class="icon-search-1"></i>', array('class' => 'btn btn-info')) . ' ' .
                    anchor(base_url('admin/update/?id=' . $value->emp_autoid), '<i class="icon-edit"></i>', array('class' => 'btn btn-warning')) . ' ' .
                    anchor(base_url('admin/delete/?id=' . $value->emp_autoid), '<i class="icon-trash-empty"></i>', array('class' => 'btn btn-danger', 'onclick' => "return confirm('ลบ?')"))
            );
        }
    ...
    function add() {
        ...
        $data['link_back'] = anchor(base_url('admin/page_emp/'), 'พนักงาน', array('class' => 'btn btn-default'));
        // run validation
        if (empty($_POST)) {
            // $data['message'] = '';
            // set common properties
            ...
            redirect(base_url('admin/page_emp/add_success'));
        }
    }
    ...
    function update() {
        ...
        $data['link_back'] = anchor(base_url('admin/page_emp/'), 'พนักงาน', array('class' => 'btn btn-default'));
        $data['link_view'] = anchor(base_url('admin/view/?id=' . $id), 'ดูข้อมูล', array('class' => 'btn btn-default'));
        // load view
        $this->load->view('empEdit', $data);
    }
    ...
    function delete() {
        ...
        redirect(base_url('admin/page_emp/delete_success'), 'refresh');
    }
    ...
    public function product_sale($offset = 0) {
        $this->check();
        if (empty($offset))
            $offset = 0;
        $sm_sale = 0;
        $config['base_url'] = base_url() . "admin/product_sale";
        ...
        if (!$foo['data'] = $this->cache->get($cacheID . $offset)) {
            $data['sm'] = $this->materials_model->select_sm_limit_sm_sale0($this->limit, $offset, $sm_sale);
            $this->cache->save($cacheID . $offset, $data['sm'], 6000);
        } else {
            $data['sm'] = $this->cache->get($cacheID . $offset, 6000);
        }
